...
async function selectFolder(folderPath) {
  ...
  if (fileList) {
    fileList.innerHTML =
      '<div class="text-center py-4">폴더를 스캔하는 중...</div>';
  }
    ...
    if (fileList) {
      ...
      if (data.files && Array.isArray(data.files) && data.files.length > 0) {
        ...
        data.files.forEach((file, index) => {
          ...
          li.className =
            "flex items-center py-2 px-3 hover:bg-gray-100 rounded cursor-pointer";
        ...
        if (data.message) {
          ...
          statusDiv.className =
            "folder-status mt-2 p-2 bg-blue-50 border-l-4 border-blue-400 text-blue-800 text-sm";
        ...
        fileList.innerHTML =
          '<div class="text-center py-4 text-gray-500">파일이 없습니다.</div>';
      }
    }
...
async function loadFile(filePath, fileType, originalMdPath) {
  ...
  document
    .getElementById("view-pdf-btn")
    .classList.remove("bg-neutral-200", "text-neutral-700");
...
async function loadPDFView(filePath, page = 1) {
  ...
  viewerContainer.innerHTML =
    '<div class="flex items-center justify-center h-full text-neutral-400"><p>PDF 로딩 중...</p></div>';
  ...
  if (rightPanel) {
    rightPanel.innerHTML =
      '<div class="flex items-center justify-center h-full text-neutral-500"><p>번역된 내용이 여기에 표시됩니다.</p></div>';
  }
...
function startTranslation(filePath) {
  ...
  setTimeout(() => {
    ...
    if (!progressBar || !progressText || !progressPercent) {
      ...
      console.log(
        "[FRONTEND] 현재 translation-container 내용:",
        rightPanel.innerHTML.substring(0, 200) + "...",
      );
    } else {
...
function updateTranslationProgress(data) {
  ...
  if (chunksInfo && data.chunks_info && Array.isArray(data.chunks_info)) {
    chunksInfo.innerHTML = data.chunks_info
      .map((chunk, index) => {
        let status = "⏳";
...
function displayTranslatedContentOrPlaceholder(filePath) {
  ...
  rightPanel.innerHTML =
    '<div class="p-4 text-gray-500">번역된 내용을 불러오는 중...</div>';
  ...
  fetch(`/api/translation-result?path=${encodeURIComponent(filePath)}`)
    ...
    .then((data) => {
      ...
      if (data.content) {
        ...
        console.log(
          "[FRONTEND] 변환된 HTML 내용 (translatedHtmlContent):",
...
async function toggleLeftPanelView(mode) {
  ...
  if (mode === "pdf") {
    ...
    if (!currentOriginalMarkdownPath) {
      viewerContainer.innerHTML =
        '<div class="flex items-center justify-center h-full text-red-500"><p>원본 마크다운 파일 경로가 없습니다.</p></div>';
      pageControls.classList.add("hidden");
      return;
    }
    viewerContainer.innerHTML =
      '<div class="flex items-center justify-center h-full text-neutral-400"><p>마크다운 로딩 중...</p></div>';
    pageControls.classList.add("hidden");
    if (viewPdfBtn && viewMdBtn) {
    ...
    try {
      const response = await fetch(
        `/api/read-file?path=${encodeURIComponent(currentOriginalMarkdownPath)}`,
      );
      ...
      // 네트워크 오류, JSON 파싱 오류, 위에서 throw된 오류 모두 처리
